...
class SubtitleGenerator:
    ...
    def generate_subtitles(self, scripts: List[dict], audio_files: List[Path], 
                          video_path: Path, language: str = "english") -> Tuple[str, str]:
        """
        ...
        try:
            ...
            for i, script_data in enumerate(scripts):
                ...
                if script_text:
                    ...
                    if i < len(audio_files):
                        ...
                        valid_audio_files.append(Path(f"/tmp/dummy_audio_{len(valid_audio_files)}.mp3"))
            ...
            srt_content = self._generate_srt_content(valid_scripts, valid_audio_files, language)
            srt_path = video_path.with_suffix('.srt')
            ...
            vtt_content = self._generate_vtt_content(valid_scripts, valid_audio_files, language)
            vtt_path = video_path.with_suffix('.vtt')
            ...
            if os.path.exists(srt_path):
                logger.info(f"SRT file created successfully, size: {os.path.getsize(srt_path)} bytes")
            else:
                logger.error(f"Failed to create SRT file at {srt_path}")
                
            if os.path.exists(vtt_path):
                logger.info(f"VTT file created successfully, size: {os.path.getsize(vtt_path)} bytes")
            else:
    ...
    def _generate_srt_content(self, scripts: List[dict], audio_files: List[Path], 
        ...
        for i, (script_data, audio_path) in enumerate(zip(scripts, audio_files)):
            # Get script content
            ...
            duration = self._get_audio_duration(audio_path) if audio_path.exists() else 5.0
            ...
            # Add subtitle entry
            srt_lines.append(str(i + 1))  # Subtitle number
    ...
    def _generate_vtt_content(self, scripts: List[dict], audio_files: List[Path], 
                             language: str = "english") -> str:
        """
