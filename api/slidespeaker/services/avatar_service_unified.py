...
class UnifiedAvatarService:
    """
    Unified avatar service that can use either HeyGen or alternative
    (DALL-E + ElevenLabs)
    """
    ...
    async def generate_avatar_video(
        ...
        **kwargs: Any,
        ...
        if provider == "heygen":
            logger.info("Using HeyGen provider for avatar video")
            return await self._generate_with_heygen(script, output_path, **kwargs)
        elif provider == "alternative":
            logger.info(
                "Using alternative provider (DALL-E + ElevenLabs) for avatar video"
            )
            return await self._generate_with_alternative(script, output_path, **kwargs)
        else:
            raise ValueError(f"Unknown provider: {provider}")

    async def _generate_with_heygen(self, script: str, output_path: Path, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if kwargs.get("fallback_to_alternative", True):
                ...
                return await self._generate_with_alternative(
                    script, output_path, **kwargs
                )
            raise
    ...
    async def get_available_providers(self) -> dict[str, bool]:
        ...
        available["heygen"] = bool(heygen_key and heygen_key != "your_heygen_api_key_here")
