...
class LocaleUtils:
    ...
    def get_display_name(cls, language: str) -> str:
        ...
        """
        display_name = cls.LANGUAGE_DISPLAY_NAMES.get(language.lower(), language.title())
    ...
    def get_supported_languages(cls) -> List[Dict[str, str]]:
        ...
        for lang_name, locale_code in cls.LANGUAGE_LOCALE_MAP.items():
            languages.append({
                ...
                "display_name": cls.LANGUAGE_DISPLAY_NAMES.get(lang_name, lang_name.title())
            })
    ...
    def normalize_language(cls, language: Optional[str]) -> str:
        ...
        if normalized not in cls.LANGUAGE_LOCALE_MAP:
            logger.warning(f"Language '{language}' not found in supported languages, defaulting to 'english'")
            return "english"
