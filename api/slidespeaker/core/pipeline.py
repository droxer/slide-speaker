...
async def _process_step(
    ...
    if state and state["steps"][step_name]["status"] in [
        ...
        if step_name == "extract_slides":
            ...
            await generate_subtitles(file_id, subtitle_language)
...
async def extract_slides(file_id: str, file_path: Path, file_ext: str) -> None:
...
async def analyze_slide_images(file_id: str) -> None:
...
async def _review_and_refine_scripts(scripts: list[dict[str, Any]], language: str = "english") -> list[dict[str, Any]]:
    ...
    review_prompts = {
        ...
        "chinese": "审查并优化以下演示文稿脚本，确保语调、风格一致，幻灯片之间过渡自然。确保语言流畅自然，保持专业而引人入胜的演示风格。",  # noqa: E501
    ...
    system_prompts = {
        ...
        "chinese": "您是一位专业的演示文稿编辑。您的任务是审查和优化演示文稿脚本的一致性、流畅性和质量，同时保持每张幻灯片的核心内容和信息。",  # noqa: E501
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                Please return the refined scripts in the same format as the original,
                with each slide clearly labeled.
                """,
                },
            ],
            max_tokens=2000,
        ...
        for line in lines:
            if (
                ...
                slide_match = re.search(
                    r"(?:Slide|幻灯片|スライド|슬라이드)\s+(\d+)", line
                )
                if slide_match:
...
async def generate_audio(file_id: str, language: str = "english") -> None:
...
async def generate_avatar_videos(file_id: str) -> None:
        ...
        if script_data and "script" in script_data and script_data["script"]:
            ...
            try:
                await avatar_service.generate_avatar_video(
                    ...
                    fallback_to_alternative=True,  # Fallback to alternative
                    # if HeyGen fails
                )
...
async def convert_slides_to_images(file_id: str, file_path: Path, file_ext: str) -> None:
...
async def generate_subtitles(file_id: str, language: str = "english") -> None:
...
async def compose_video(file_id: str, file_path: Path) -> None:
    ...
    if scripts_data and generate_subtitles:
        try:
            ...
            if not audio_files_data:
                ...
                # Create a list of dummy paths for estimated durations (5 seconds each)
                # The subtitle generator will handle non-existent files by "
