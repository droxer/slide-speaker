...
class MasterWorker:
    ...
    def start_worker_process(self, task_id: str) -> subprocess.Popen:
    ...
    def cleanup_workers(self):
        ...
        for task_id, process in self.worker_processes.items():
            try:
                ...
                logger.warning(f"Worker for task {task_id} did not terminate, killing...")
                process.kill()
                process.wait()
    ...
    def check_completed_workers(self):
        ...
        for task_id, process in list(self.worker_processes.items()):
            if process.poll() is not None:  # Process has completed
                ...
                logger.info(f"Worker for task {task_id} completed with return code {process.returncode}")
                ...
                if process.returncode == 0:
                    ...
                    task_queue.update_task_status(task_id, "failed", 
