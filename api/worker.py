            ...
            try:
                ...
                if task and "status" in task:
                    ...
                    if task.get("status") == "cancelled":
                        logger.info(f"Task {self.task_id} was cancelled, stopping monitoring")
                        break
                        
                    # Also check for immediate cancellation
                    if task_queue.is_task_cancelled(self.task_id):
                        logger.info(f"Task {self.task_id} was cancelled (immediate check), stopping monitoring")
                        break
...
async def process_task(task_id: str) -> bool:
    ...
    try:
        ...
        task_queue.update_task_status(task_id, "processing")
        ...
        logger.info(f"Task {task_id} parameters extracted - file_id: {file_id}, file_ext: {file_ext}")
        ...
        logger.info(f"Task {task_id} starting presentation processing for file {file_id}")
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            task_queue.complete_task_processing(task_id)
